...
public class ShareRouteDaoImpl  implements ShareRouteDao {
	...
	public void saveRoutePic(String id, File pic) {
		String newFilename = id;
		DB db = mongoTemplate.getDb();  
		GridFS gfsRoute = new GridFS(db,"routepic");
		try {
			GridFSInputFile gfsFile = gfsRoute.createFile(pic);
			gfsFile.setFilename(newFilename);
			gfsFile.save();
		} catch (IOException e) {
			// TODO Auto-generated catch block
	...
	public GridFSDBFile getRoutePic(String id) {
        ...
        try {  
            ...
            String the_Filename = id;
            // 获取fs的根节点  
            GridFS gridFS = new GridFS(db, "routepic");  
            GridFSDBFile dbfile = gridFS.findOne(the_Filename);  
            if (dbfile != null) {  
                return dbfile;  
            }  
	...
	public void deleteRoutePic(String id) {
		...
		DB db = mongoTemplate.getDb();  
		GridFS gfsPhoto = new GridFS(db,"routepic");
		try {
